...
if (isset($_POST)){//} && !empty($_POST['type']) && !empty($_POST['amount'])){
    ...
    if (isset($_FILES['receipt']['tmp_name'])){
      ...
      $imageFileType = $imageFileType = pathinfo(basename($_FILES["receipt"]["name"]),PATHINFO_EXTENSION);
      ...
      if (move_uploaded_file($receipt, $target_file) !== True){
        $file_name = NULL;
      }
    } else {
        $file_name = NULL;
    ...
    //     $sql = "INSERT INTO `test`.`transaction` (user_id, team_id, place, description, amount) VALUES ('$user_id', '$team_id', 'PASS', 'test', '25')";
    //     mysqli_query($conn, $sql);
    // } else {
    //     $amount = $_POST['amount'];
    //     $sql = "INSERT INTO `test`.`transaction` (user_id, team_id, place, description, amount) VALUES ('$user_id', '$team_id', 'FAIL', '$amount', '25')";
    //     mysqli_query($conn, $sql);
    // }
  }
//
